...
"""
Extract domain-agnostic arguments from text using Amazon Bedrock
This version focuses on extracting substantive claims about the subject matter itself
"""
...
class BedrockClient:
    ...
    def extract_substantive_claims(self, text, chunk_id="chunk1"):
        """Extract substantive claims from text using Bedrock"""
        prompt = """
        Analyze the following text and extract the key substantive claims being made about the subject matter.

        Focus on:
        1. Claims that represent core arguments or positions about the subject matter itself
        2. Claims that could be debated, challenged, or require evidence to support
        3. Claims that represent important principles, rules, or interpretations in the domain
        4. Claims that establish relationships, causality, or requirements

        Avoid:
        1. Claims about the document itself (e.g., "This document describes...")
        2. Claims about the author's intentions or methods
        3. Simple factual statements that aren't being argued for
        4. Restatements of the document's structure or organization

        For each substantive claim:
        1. Formulate it as a clear, declarative statement representing a position being taken
        2. Make sure it's a complete thought that someone could agree or disagree with
        3. Ensure it represents a meaningful assertion about the subject matter
        4. Identify specific text spans that support this claim

        Examples of good substantive claims:
        - "Pilots are ultimately responsible for the safe operation of their aircraft regardless of ATC clearances."
        - "Climate change is primarily caused by human activities that increase greenhouse gas concentrations."
        - "Free markets are more efficient at resource allocation than centrally planned economies."
        - "The First Amendment protects symbolic speech as well as verbal expression."

        Format your response as a JSON array of objects with these fields:
        - id: A short identifier (e.g., "claim1")
        - claim: The substantive claim formulated as a declarative statement
        - description: Brief explanation of the claim's significance
        - text_evidence: Direct quotes from the text where this claim is made or supported
        - chunk_ids: Array of chunk identifiers where this claim appears

        TEXT:
        {text}

        SUBSTANTIVE CLAIMS (JSON format):
        """
    ...
    def extract_positions(self, text, claim):
        ...
        4. Identify specific text that supports this position
...
class ArgumentModel:
    """Class to represent the argument model with substantive claims"""
...
def process_text(text, document_id="sample", chunk_id="chunk1"):
    ...
    try:
        ...
        # Extract substantive claims
        logger.info("Extracting substantive claims...")
        claims = bedrock.extract_substantive_claims(text, chunk_id)
        logger.info(f"Extracted {len(claims)} substantive claims")
...
def main():
    parser = argparse.ArgumentParser(description='Extract domain-agnostic arguments from text')

...
"""
Extract substantive claims and arguments from a text sample using Amazon Bedrock
This version focuses on extracting claims about the subject matter, not about the paper itself
"""
...
logger = logging.getLogger("substantive_claim_extractor")
...
class BedrockClient:
    ...
    def extract_substantive_claims(self, text, chunk_id="chunk1"):
        ...
        prompt = """
        Analyze the following text and identify the substantive claims that the authors are arguing for.

        Focus on:
        1. Claims about the subject matter itself (not about the paper)
        2. Claims that represent the authors' position on how things work or should work
        3. Claims that could be debated or that the authors are providing evidence for

        Avoid:
        1. Claims about what the paper is doing or the authors' intentions (e.g., "This paper presents...")
        2. Methodological statements about the research approach
        3. Simple factual statements that aren't being argued for

        For each substantive claim:
        1. Formulate it as a clear, declarative statement representing a position the authors are taking
        2. Provide a brief description of why this is an important claim in the context of the paper
        3. Identify specific text spans that show the authors making or supporting this claim
        ...
        - text_evidence: Direct quotes from the text where the authors make or support this claim
...
def main():
    parser = argparse.ArgumentParser(description='Extract substantive claims and arguments from text')
    ...
    try:
        ...
        # Save results
        output_path = args.output or f"output/{document_id}_substantive_claims.json"
